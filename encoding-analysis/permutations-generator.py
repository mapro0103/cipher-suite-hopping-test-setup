...
def generate_permutations_file(output_file="permutations_scenario2.json", 
    ...
    for idx, perm_group in enumerate(permutation_groups[:total_entries], 1):  # Start ID from 1
        if idx <= max_ascii_entries:
            ...
            # For extra entries beyond ASCII limit: Start at 256 and increment upwards for all values
            # Calculation based on the excess index (idx - max_ascii_entries)
            extra_idx = idx - max_ascii_entries
            if ascii_dimensions == 1:
                ascii_values = [255 + extra_idx]
            elif ascii_dimensions == 2:
                # For 2D: both values start at 256 and increment
                ascii_values = [255 + extra_idx, 255 + extra_idx]
            else:  # ascii_dimensions == 3
                # For 3D: all three values start at 256 and increment
                ascii_values = [255 + extra_idx, 255 + extra_idx, 255 + extra_idx]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output', type=str, default='permutations.json', 
